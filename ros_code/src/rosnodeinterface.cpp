  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ros_code\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(ros_code_M, (NULL));
        ros_code_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(ros_code_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(ros_code_M)
                == (NULL)) || rtmGetStopRequested(ros_code_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(ros_code_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
